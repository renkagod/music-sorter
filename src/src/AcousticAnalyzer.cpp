    ...
    for (size_t i = 0; i < fpResults.size(); ++i) {
        for (size_t j = i + 1; j < fpResults.size(); ++j) {
            ...
            if (std::abs(f1.duration - f2.duration) <= 5.0) {
                ...
                // Auto-delete applies to:
                // 1. Exact 96%+ identical wave matches (any formats)
                // 2. FLAC vs MP3 pairs of the SAME audio at 85%+ (MP3 auto-deleted — same track in lossy/lossless)
                // 3. Explicit "- Copy" folders that are 96%+ exact duplicates
                // Anything 70-95% same-format goes to A/B comparison for user decision!
                if (sim >= 0.96 || (isFlacMp3Pair && sim >= 0.85) || (isExplicitCopyFolder && sim >= 0.96)) {
                    ...
                    std::string autoDelRel = fs::relative(autoDelPath, baseDirectory).string();
                    std::string keptPath = (autoDelPath == f1.path) ? f2.path : f1.path;
                    std::string keptRel = fs::relative(keptPath, baseDirectory).string();
                    LOG_INFO("[AUTO-DELETE MATCH " + std::to_string((int)(sim * 100)) + "%] Deleting: " + autoDelRel + " | Keeping: " + keptRel);
