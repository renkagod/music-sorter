...
def process_album_folder(root_path, files):
    ...
    for f in audio_files:
        ...
        try:
            ...
            album_clean = re.sub(r'\[[^\]]*\]|\{[^\}]*\}|\(\d{4}[^\)]*\)', '', album_clean).strip()
            album_clean = re.sub(r'[\s\-_/\\,.\u2044\u2215\u3013\uFF5E]+', ' ', album_clean).strip()
...
def run_organization(target_album=None):
    logging.info("==================================================")
    logging.info("  STARTING BATCH TAGGING & FILE ORGANIZATION      ")
    logging.info("==================================================")

    for root, _, files in os.walk(TOSORT_DIR):
        if target_album and target_album.lower() not in root.lower():
            continue
        process_album_folder(root, files)

    logging.info("==================================================")
    logging.info("  BATCH ORGANIZATION COMPLETE. APPLYING FALLBACK ")
    logging.info("==================================================")
